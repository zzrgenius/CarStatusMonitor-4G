...
//float pitch,roll,yaw;
 ...
 static void SetAll_IMUdata(void );
/* Get data from MPL.
...
//					rs_485_buf[0] = 0;
//					sprintf(rs_485_buf,"Linear Accel: %7.5f %7.5f %7.5f\r\n",float_data[0], float_data[1], float_data[2]);
//					message_pack(&g_tx_messgage,0,(uint8_t*)rs_485_buf,strlen(rs_485_buf)); 
//					if((float_data[2] > 8) || (float_data[2] < -8))
//					{
//						LED_Toggle(LED1);
//						LED_Toggle(LED3);
//					//	
    ...
    if (hal.report & PRINT_GRAVITY_VECTOR) {
            if (inv_get_sensor_type_gravity(float_data, &accuracy,
            	...
            	printf("Gravity Vector: %7.5f %7.5f %7.5f\r\n",
            			float_data[0], float_data[1], float_data[2]);  //MPL_LOGI
    }
   ...
   // char c = USART_ReceiveData(USART1);
    ...
    case 'f':
        ...
        if (hal.dmp_on) {
             ...
             * 200Hz, an d the DMP is configured to downsample the FIFO output
...
{ 
    ...
    struct int_param_s int_param;
		static unsigned long sendticks;
	static unsigned long alarm_ticks;
#ifdef COMPASS_ENABLED
...
#endif
        if (new_data) {
			float_t float_data[3],average_out;
 			//int8_t accuracy;
			static uint8_t alram_flag;
...
//			{
//				average_out = filter_average(float_data[2]);
//				if( filter_limit(average_out))
//				{
//					alram_flag = 1;
//					get_tick_count(&alarm_ticks); 
//				}
//			 
//			}
...
static void SetAll_IMUdata(void )
{
	...
	inv_get_sensor_type_compass(&data[3],&accuracy,  (inv_time_t*)&timestamp);
	inv_get_sensor_type_linear_acceleration(float_data, &accuracy, (inv_time_t*)&timestamp);
	mav_send_imu_msg((int16_t)(float_data[0]*100),(int16_t)(float_data[1]*100),(int16_t)(float_data[2]*100),
