...
extern osMessageQId q_msg_received_Id;
...
uint8_t SIM_power_reset(void)
{
  ...
  *
 */
	HAL_GPIO_WritePin(SIM_RST_GPIO_Port_GPIO_Port, SIM_RST_GPIO_Port_Pin, GPIO_PIN_SET);
	HAL_Delay(150U);
	HAL_GPIO_WritePin(SIM_RST_GPIO_Port_GPIO_Port, SIM_RST_GPIO_Port_Pin, GPIO_PIN_RESET);
...
static void msgReceivedCallback(void)
{
 ...
 // MsgReceived++;
  ...
  if (osMessagePut(q_msg_received_Id, 1, 0U) != osOK)
  {
